...
- `stableStringify`：内部结构重构，**语义等价、输出逐字节不变**——第一相（toJSON 与 replacer）由遍历闭包内的嵌套函数提炼为模块级函数 `resolveReplacement`，`replacer` 改为显式入参，不再隐式捕获调用级状态（`stringify` 闭包少一个捕获项）。验收：差分对照重构前实现 **12410 次比对 → 0 处不一致**——语料 = 803 例结构输入 + 19 例副作用探针 + 20 例环形态，叉乘 15 种选项形态；比对内容除返回值与异常（类型 + 消息）外，还含**可观察副作用**：属性读取次数与顺序、Proxy `get`/`has`/`ownKeys`/`getOwnPropertyDescriptor`/`getPrototypeOf` 陷阱次数与顺序、装箱 Number/String 的 `valueOf`/`toString` 调用次数、`Set.prototype.has`/`add`/`delete` 调用次数、toJSON 的 this 绑定与收到的 key、cmp getter 的同一性、replacer 的 `(parent, key)` 序列与 this 绑定。台架以 16 处故意改坏做过阳性对照（含 V8 报错文本、Set 调用次数、属性读次、Proxy 陷阱次数、replacer this 绑定等），逐条报红后撤回。微基准（同进程配对 + 顺序平衡 + 符号检验，81 轮）四种形态三轮均无复现的变慢。同轮另尝试把环路径收归 `CyclePath` 类以消除 `onPath`/`pathKeys` 的手工配对，因深嵌套 depth=200 稳定变慢约 1% 而按预设门槛回退、未合入（见 `.scratch/stable-stringify-cyclepath-ownership/`，补丁 `f1-cyclepath.patch` 可重新应用）
...
- `stableStringify`：装箱品牌判定与取值所用的原语全部改为**模块加载时捕获**——此前只捕获 `Object.getPrototypeOf` / `Object.prototype.toString`（注释自陈理由是「可被调用方改写，捕获后判定不受其影响」），而真正裁决品牌的四个 `X.prototype.valueOf` 与取值用的 `Number` / `String` / `Symbol.toStringTag` 都是调用时实时解析，「同一份判定里两个捕获、其余没捕获」自相矛盾。修掉四种可观察错值：改写 `Number.prototype.valueOf` 后 `Object.create(Number.prototype)` 输出 `"42"`（原生 `{"a":1}`）、改写 `Boolean.prototype.valueOf` 输出 `"true"`、改写 `String.prototype.valueOf` 抛 TypeError、改写 `BigInt.prototype.valueOf` 抛「Do not know how to serialize a BigInt」——现四者均与原生一致。**`value` 路径刻意保持实时求值**：实例级改写的 `valueOf` / `toString` 仍按 ToNumber / ToString 参与取值、抛错仍向调用方外溢（既有用例钉住的 TypeError 语义不变）。验证：新增 8 条用例，四类原型改写与全局 `Number` / `String` / `Symbol` 替换均与原生同进程**逐字节比对**；守卫对照（剔除本轮预期变化的两类输入）841 次比对 → 0 处不一致；`test` 281 条全绿
- `stableStringify`：循环引用的 TypeError 文本补齐为 **V8（Node）口径**——在原有首行之后追加 `--> starting at object with constructor 'X'`、每跳一行 `| property 'k' -> object with constructor 'Y'`（数组元素为 `index N`）、末行 `--- property 'k' closes the circle`（空串键一律渲染为 `<anonymous>`）。构造器名沿原型链取**自有 data 属性** `constructor` 的函数名：getter 一律不触发、非函数值继续上行、走完回落 `'Object'`。两处与 V8 不一致（V8 读函数内部名，userland 取不到）：函数的 `name` 被改写时、bound 函数（V8 会跳过它）——均记入文件内已知边界。为不拖慢热路径，「节点 → 边 key」不用 `Map` 而取「`Set` + 并行 key 数组」（同进程交替测量下 `Map` 在宽对象上 +10%，该结构则与改动前持平）。验证：新增 20 条用例逐形状与原生同进程**逐字节比对**（根自引用 / 多跳 / 数组索引 / 类实例 / 空键 / Proxy / `Object.create(null)` / `toJSON` 环 / 下标 12 等）；微基准每进程单实现各 3 次取最小值——深嵌套 +6.9%、宽对象 +4.7%、大数组 +2.2%、混合图 -2.7%（另一轮重测方向不一致，除深嵌套外均落在波动区间内）；`createFetch` 中钉住整条文本的断言随本次变更更新为「包含首行」（该用例钉的是「循环配置归一化为 `-4`」，文本格式由新增用例钉住）
- `stableStringify` 文档校正（**无行为变更**）：`ReplacerFunc` 的「签名与原生 `JSON.stringify` 的 replacer 一致」与公开 API 文档的「签名为 `(parent, key, value)`」互相矛盾，且会引导调用方照原生习惯写两参（运行时静默错位：第一参收到父对象、第二参收到 key，不报错）——改为「**比原生多一个前导 `parent` 参数**」，并写明两参写法会静默错位、类型上拦不住、运行期不做 `Function.length` 嗅探的理由；`space` 注释点明装箱 Number/String 属运行期兼容、**类型只收 `string | number`**（与原生 lib 的类型一致，此前用例只能 `as any`）；`cmp` 注释点明比较器每次比较都重新取 `node[a]` / `node[b]`（重读与逐次分配属既有行为，改动会动到可观察语义，另立 `.scratch/stableStringify-cmp-reads/issues/01-cmp-rereads.md` 排期）。CONTEXT.md：「稳定序列化」词条补「判定与取值所用的原语在模块加载时捕获」，「循环引用」词条补报错文本的 V8 口径与非 V8 引擎（JSCore）的差异
