...
- **稳定序列化（stable serialization）**：确定性 JSON 序列化——对象键按 UTF-16 码点排序，相同内容恒产出相同字符串；支持自定义排序比较、replacer、循环引用标记（`cycles`）、空容器紧凑输出（`collapseEmpty`）；默认遇循环引用抛 TypeError。与原生 `JSON.stringify` 的差异：类数组整数键（如 `"2"`/`"10"`）不按数值优先排序（原生会将其排在最前），而是与其余键统一按码点序排序；确定性不受影响，但跨工具哈希比对时需注意。
