...
- **决策边界**：本决策仅收窄 fetch 结果路径的注入面——`createFetch` 的返回值（`request`/快捷方法）不得被任何调用方代码的抛错污染。核心域 `createEventEmitter` 的 `onError` 注入（处理器抛错上报回调）保留：其抛错会中断剩余处理器并向 `emit` 调用方传播（注入方责任，语义见 CONTEXT「事件发射器」词条），但不进入 fetch 结果路径（`createFetch` 内部发射器不注入 `onError`，走默认 `console.error`），不构成「返回值报错」风险（拦截器抛错由错误归一化收口为 `-4`，属契约内行为，非注入面问题）。
